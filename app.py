...
from sqlalchemy import or_, func, text # (추가) text 임포트
...
def import_excel():
    if request.method == 'POST':
        if 'excel_file' not in request.files: flash('오류: 파일 선택 안됨.', 'error'); return redirect(url_for('index'))
        file = request.files['excel_file']
        if file.filename == '': flash('오류: 파일 선택 안됨.', 'error'); return redirect(url_for('index'))
        ...
        else: flash('오류: 엑셀 파일만 업로드 가능.', 'error'); return redirect(url_for('index'))
    return redirect(url_for('index'))
...
def product_detail(product_number):
    ...
    if product.product_name:
        search_words = product.product_name.split(' '); search_term = search_words[-1]
...
def barcode_search():
    ...
    # DB의 barcode에서도 하이픈 제거 후, scanned_clean으로 시작하는지 확인
...
# (*** 2. 신규: Neon DB 깨우기 함수 ***)
def keep_db_awake():
    """Neon DB가 잠들지 않도록 간단한 쿼리를 실행합니다."""
    try:
        with app.app_context():
            # 가장 간단한 쿼리 실행 (데이터 변경 X)
            db.session.execute(text('SELECT 1'))
            print("Neon DB keep-awake query executed.")
    except Exception as e:
        print(f"Error executing keep-awake query: {e}")

# (*** 3. 신규: 스케줄러 설정 및 시작 ***)
if os.environ.get('WERKZEUG_RUN_MAIN') != 'true':
    scheduler = BackgroundScheduler(daemon=True)
    scheduler.add_job(keep_db_awake, 'interval', minutes=4)
    scheduler.start()
    print("APScheduler started to keep Neon DB awake.")
