...
from werkzeug.utils import secure_filename # 파일 업로드 처리용
...
# (*** 3. 파일 업로드 설정 추가 ***)
app.config['UPLOAD_FOLDER'] = '/tmp' # 임시 저장 폴더 (Render는 /tmp 사용 가능)
...
def product_detail(product_number):
    ...
    if product.product_name: search_words = product.product_name.split(' '); search_term = search_words[-1]
...
def ocr_upload():
    """ 사진 파일을 받아서 서버에서 OCR 처리 후 상품 검색 """
    ...
    if file:
        try:
            ...
            # pytesseract로 OCR 실행 (한국어+영어)
            # --psm 6: Assume a single uniform block of text. (통 이미지)
            ...
            # 'M'으로 시작, 알파벳/숫자/하이픈 허용, 최소 5글자 이상인 단어 찾기
            # \b: 단어 경계, M[A-Z0-9-]{4,}: M + (A-Z or 0-9 or -) 4번 이상
            ...
            if matches:
                ...
                # 찾은 품번으로 상품 검색 (/text_search 로직 재사용)
                ...
                return jsonify({'status': 'error', 'message': 'OCR 결과에서 품번 패턴(M...)을 찾지 못했습니다.'}), 400

        except Exception as e:
            print(f"Server OCR Error: {e}") # 디버깅용 로그
            return jsonify({'status': 'error', 'message': f'서버 OCR 처리 중 오류 발생: {e}'}), 500
