...
# (*** 1. Google Cloud Vision 라이브러리 추가 ***)
from google.cloud import vision
from google.oauth2 import service_account # 서비스 계정 키 사용

# (*** pytesseract, Pillow, werkzeug.utils 삭제 ***)
# import pytesseract
...
# (*** 2. Google Cloud 인증 정보 경로 설정 ***)
# Render 환경 변수에서 키 파일 경로를 읽어옴
...
if GCP_CREDENTIALS_PATH and os.path.exists(GCP_CREDENTIALS_PATH):
    ...
    # 로컬 개발용: 키 파일이 코드와 같은 폴더에 있다고 가정 (GitHub에 올리면 안됨!)
...
class Product(db.Model):
    # ... (이전과 동일) ...
    __tablename__ = 'products'; product_number = db.Column(db.String, primary_key=True); product_name = db.Column(db.String, nullable=False); is_favorite = db.Column(db.Integer, default=0)
...
# (*** 3. 수정된 /ocr_upload 함수 - Google Vision AI 사용 ***)
