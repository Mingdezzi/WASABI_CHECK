    ...
    if request.method == 'POST':
        ...
        if file and (file.filename.endswith('.xlsx') or file.filename.endswith('.xls')):
            try:
                ...
                if not all(col in df.columns for col in required_cols): flash(f"엑셀 컬럼명 오류. 필수 10개 컬럼 확인: {required_cols}", 'error'); return redirect(url_for('index'))
            ...
            except Exception as e: db.session.rollback(); flash(f"임포트 중 심각한 오류 발생: {e}", 'error')
            return redirect(url_for('index'))
        else: flash('오류: .xlsx 또는 .xls 엑셀 파일만 업로드 가능.', 'error'); return redirect(url_for('index'))
    return redirect(url_for('index'))
...
def product_detail(product_number):
    ...
    if product is None: flash("상품을 찾을 수 없습니다.", 'error'); return redirect(url_for('index'))
...
def update_stock():
    ...
    try:
        ...
        if item is None: return jsonify({'status': 'error', 'message': '해당 상품(바코드) 없음.'}), 404
